...
export function CategoryBadge({ category }: { category: { name: string; color: string } }) {
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium ring-1 ring-inset ${colorMap[category.color] ?? colorMap.slate}`}
    >
      <span
        className="h-1.5 w-1.5 rounded-full"
        style={{ backgroundColor: categoryHex[category.color] ?? categoryHex.slate }}
      />
      {category.name}
    </span>
  );
